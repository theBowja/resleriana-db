...
Prerequisite: #Local development 
...
Which server to pull images from.
...
Which platform to pull images from. The images from "StandaloneWindows64" will be of noticeable higher quality than images from the mobile platforms.
...
Path to the folder where the images will be outputted. If it is a relative path, then it will be relative to the current working directory.
...
Format of the image to convert to. Images from Unity bundles are in a compressed format like BC7. They are decompressed into RGBA before being converted into a more common format. Png images are double the size of webp images. Webp images will go through lossy conversion. You can change the quality of the conversion by modifying the python script at `./tools/UnityPyScripts/exportAssets.py`. Bulk image conversion will take time depending on the desired size and quality of the final image.
...
Whether or not to skip downloading the catalog and bundles. This may be useful for saving time if you've already downloaded these from running the command previously and know that the game has not been updated since. The script downloads the catalog in order to create a list of Unity bundles that contain Texture2D assets. This is list is saved in the file: `./resources/[server]/[platform]/bundlenames_all_texture2d.txt`. The bundles are then downloaded into the folder: `./resources/[server]/[platform]/bundles`.
...
Regex on image name to filter which image to save. If you are running the command in PowerShell (and maybe others), you will need to prepend the stop-parsing token (`--%`) to the arguments list and add quotes around the regex in order for regex to be passed successfully into the script. Otherwise, characters like '^' will be stripped out.
